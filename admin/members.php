...
if($_GET['action']=='active')
{
	...
	for($i=0;$i<=count($getid);$i++)
	{
		...
		$getrow=mysqli_query($conn,"select * from tbl_invite_earn where user_id='".$getid[$i]."'");
		$rowcount=mysqli_num_rows($getrow);
		
		if($rowcount==1)
		{
			mysqli_query($conn,"update tbl_invite_earn set status='1' where user_id ='".$getid[$i]."'");
		}
	}
...
function getuser(method)
{
	...
	$.ajax({
        url: 'ajaxmember.php',
        type: 'GET',
        data: { types: types,method:method,page:'all_member'} ,
...
/*	$('#member').DataTable( {
	...
	*/
	...
	$.ajax({
        url: 'ajaxmember.php',
        type: 'GET',
        data: { types: types,method:'all',page:'all_member'} ,
        contentType: 'application/json; charset=utf-8',
        success: function (response) {
...
<body>
 ...
 <div id="result_para">
 <table style="border: 0px solid #dddddd;background-color:#FFFFFF">
 <thead>
 <th></th>
 <th></th>
 <th></th>
 ...
 $limit = 10;  
	...
	if($_GET['type']=='all')
	$query=mysqli_query($conn,"select * from tbl_user where delete_status=0 ORDER BY user_id ASC LIMIT $start_from, $limit");								
	else if($_GET['type']=='paid')
	$query=mysqli_query($conn,"select * from tbl_user where paid_status=1 and delete_status=0 ORDER BY user_id ASC LIMIT $start_from, $limit");
	else if($_GET['type']=='male')
	$query=mysqli_query($conn,"select * from tbl_user where gender='male' and delete_status=0 ORDER BY user_id ASC LIMIT $start_from, $limit");
	
	else if($_GET['type']=='female')
	$query=mysqli_query($conn,"select * from tbl_user where gender='female' and delete_status=0 ORDER BY user_id ASC LIMIT $start_from, $limit");
	else if($_GET['type']=='today')
	$query=mysqli_query($conn,"SELECT * FROM tbl_user WHERE DATE(profile_created_by) = CURDATE() and delete_status=0 ORDER BY user_id ASC LIMIT $start_from, $limit");
	else if($_GET['type']=='lweek')
	$query=mysqli_query($conn,"select * from tbl_user where  profile_created_by >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) and delete_status=0 ORDER BY user_id ASC LIMIT $start_from, $limit");					
	else if($_GET['type']=='lmonth')
	$query=mysqli_query($conn,"select * from tbl_user where  profile_created_by >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)  and delete_status=0 ORDER BY user_id ASC LIMIT $start_from, $limit");
...
<?php  
if($_GET['type']=='all')
	$query=mysqli_query($conn,"select COUNT(user_id) from tbl_user where delete_status=0 ");								
	else if($_GET['type']=='paid')
	$query=mysqli_query($conn,"select COUNT(user_id) from tbl_user where paid_status=1 and delete_status=0 ");
	else if($_GET['type']=='male')
	$query=mysqli_query($conn,"select COUNT(user_id) from tbl_user where gender='male' and delete_status=0 ");
	
	else if($_GET['type']=='female')
	$query=mysqli_query($conn,"select COUNT(user_id) from tbl_user where gender='female' and delete_status=0 ");
	else if($_GET['type']=='today')
	$query=mysqli_query($conn,"SELECT COUNT(user_id) FROM tbl_user WHERE DATE(profile_created_by) = CURDATE() and delete_status=0 ");
	else if($_GET['type']=='lweek')
	$query=mysqli_query($conn,"select COUNT(user_id) from tbl_user where  profile_created_by >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) and delete_status=0 ");					
	else if($_GET['type']=='lmonth')
	$query=mysqli_query($conn,"select COUNT(user_id) from tbl_user where  profile_created_by >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)  and delete_status=0 ");
...
for ($i=1; $i<=$total_pages; $i++) {  
             $pagLink .= "<div align=right><li><a href='members.php?type=".$_GET['type']."&page=".$i."'>".$i."</a></li></div>";  
};  
echo $pagLink . "</ul></nav>";  
?>
...
$('.pagination').pagination({
		...
		hrefTextPrefix : "members.php?type=<?php echo $_GET['type'];?>&page="
    });
	});
</script>
                    </div>
