...
<body>
 ...
 <th></th>
  ...
  <?php 
 if($_GET['type']=='all')
	$query=mysqli_query($conn,"select * from tbl_user where delete_status=0 limit 0,10");								
	else if($_GET['type']=='paid')
	$query=mysqli_query($conn,"select * from tbl_user where paid_status=1 and delete_status=0 limit 0,10");
	else if($_GET['type']=='male')
	$query=mysqli_query($conn,"select * from tbl_user where gender='male' and delete_status=0 limit 0,10");
	else if($_GET['type']=='female')
	$query=mysqli_query($conn,"select * from tbl_user where gender='female' and delete_status=0 limit 0,10");	
	else if($_GET['type']=='today')
	$query=mysqli_query($conn,"SELECT * FROM tbl_user WHERE DATE(profile_created_by) = CURDATE() and delete_status=0 limit 0,10");
	else if($_GET['type']=='lweek')
	$query=mysqli_query($conn,"select * from tbl_user where  profile_created_by >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) and delete_status=0 limit 0,10");					
	else if($_GET['type']=='lmonth')
	$query=mysqli_query($conn,"select * from tbl_user where  profile_created_by >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)  and delete_status=0 limit 0,10");
...
<a href="#" class="btn btn-custom  btn-block waves-effect waves-light">Renew Membership</a>
 ...
 <?php } ?>
                ...
                <input type="hidden" id="result_no" value="10">
					<input type="hidden" id="gettype" value="<?php echo $_GET['type']; ?>">
                </div>
    ...
    background-repeat: repeat-x;
			...
			<div align="center">
				<input type="button" style="background-color:#de3c94" id="load" value="Load More Results" >
				</div>
